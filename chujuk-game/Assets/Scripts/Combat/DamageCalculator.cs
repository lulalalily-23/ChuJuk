...
public static class DamageCalculator
{
    ...
    public static int CalculateDamage(CharacterStats attacker, CharacterStats defender, float skillMultiplier = 1f)
    //세번째 인자는 기본값 1로 설정(일반적으로는 앞 두인자만 채워도 작동). 나중에 스킬등으로 공격계수를 추가해야할 일을 위해 임시적으로 만듦.
    {
        float rawDamage = attacker.currentAttackPower * attacker.attackMultiplier * skillMultiplier - defender.currentDefense * defender.defenseMultiplier;
